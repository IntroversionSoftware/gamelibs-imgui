...
bool    ImGui_ImplOpenGL3_Init(const char* glsl_version)
{
    ...
    // If we are only going to use a single context, we can avoid the overhead
    // of re-creating the VAO every time.
    bd->SingleContext = true;
...
static void ImGui_ImplOpenGL3_SetupRenderState(ImDrawData* draw_data, int fb_width, int fb_height)
{
    ...
    if (!GLAD_GL_ES_VERSION_3_0 && bd->GlVersion >= 310)
    ...
    extern int IsSurfaceFlipped();
    const int surface_flipped_y = IsSurfaceFlipped();
#endif
...
#if defined(GL_CLIP_ORIGIN)
    if (!clip_origin_lower_left && !surface_flipped_y) { float tmp = T; T = B; B = tmp; } // Swap top and bottom if origin is upper left
#endif
    ...
    if (bd->VaoHandle) {
        ...
        GL_CALL(glGenVertexArrays(1, &bd->VaoHandle));
        glBindVertexArray(bd->VaoHandle);
#endif
...
void    ImGui_ImplOpenGL3_RenderDrawData(ImDrawData* draw_data)
{
...
#ifdef IMGUI_IMPL_OPENGL_MAY_HAVE_PRIMITIVE_RESTART
    GLboolean last_enable_primitive_restart = (bd->GlVersion >= 310 && !GLAD_GL_ES_VERSION_3_0) ? glIsEnabled(GL_PRIMITIVE_RESTART) : GL_FALSE;
#endif
    ...
    if (!bd->SingleContext) {
        ...
        GL_CALL(glDeleteVertexArrays(1, &bd->VaoHandle));
        bd->VaoHandle = 0;
    }
#endif
...
#ifdef IMGUI_IMPL_OPENGL_MAY_HAVE_PRIMITIVE_RESTART
    if (!GLAD_GL_ES_VERSION_3_0 && bd->GlVersion >= 310) { if (last_enable_primitive_restart) glEnable(GL_PRIMITIVE_RESTART); else glDisable(GL_PRIMITIVE_RESTART); }
#endif
...
bool    ImGui_ImplOpenGL3_CreateDeviceObjects()
{
    ...
    // Generate the VAO on first use, if we're in single-context mode
    bd->VaoHandle = 0;
